...
write('Seleccione una operacion:'), nl, 
write('1: suma'), nl, 
write('2: multiplicacion'),nl, 
write('3: division'),nl, 
write('4: resta'),nl, 
write('Cualquier otro para salir'), 
read(O), 
O < 5 ->  write('Ingrese el primer numero'), read(X),nl, 
write('Ingrese el segundo numero'), read(Y), 
( O =:= 1 -> R is X+Y; 
( O =:= 2 -> R is X*Y; 
( O =:= 3 -> R is X/Y; 
( O =:= 4 -> R is X-Y)))),
write('El resultado es: '), write(R), calculadora(R).
